...
static void mavlink_test_esc_status(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
	...
	mavlink_msg_esc_status_decode(&msg, &packet2);
        ...
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_esc_status_pack(system_id, component_id, &msg , packet1.current , packet1.rpm );
	mavlink_msg_esc_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_esc_status_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.current , packet1.rpm );
	mavlink_msg_esc_status_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
	...
	mavlink_msg_esc_status_decode(last_msg, &packet2);
        ...
        memset(&packet2, 0, sizeof(packet2));
	mavlink_msg_esc_status_send(MAVLINK_COMM_1 , packet1.current , packet1.rpm );
	mavlink_msg_esc_status_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
